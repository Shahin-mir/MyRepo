...
//*****************************************************************
		List<Document> lDoc = new ArrayList<Document>();//list to add doc
		ScoreDoc tempscoreDoc = null;
		...
		if (identifiedField.toString().length() > 0) {
 ...
 //*************************************************
		...
		for( ScoreDoc scoreDoc: topScoreDocCollector.topDocs().scoreDocs) { 
			...
			lDoc.add(document);
			tempscoreDoc = scoreDoc;
			
		        }//end forloop search	
			}
			if(lDoc!=null && lDoc.size()>0){
			System.out.println("Found:");
			Document document = lDoc.get(lDoc.size()-1);
			System.out.printf("Concept Object: conceptId=%s, conceptActive=%s, FSN=%s, relativeIdnType=%s, destinationConceptId=%s, relationType=%s,synonymDetails=%s,score=%f\n",
					document.get("conceptId"), 
					document.get("conceptActive"),
					document.get("FSN"),
					document.get("relativeIdnType"),
					document.get("destinationConceptId"),
					document.get("relationType"),
					document.get("synonymDetails"),tempscoreDoc.score);}else {System.out.println("No results Found !");}
		  } // end if
